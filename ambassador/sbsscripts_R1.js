...
$("#ambsdr-form .submit-btn,.mobile-submit-btn").on("click", function () {
	...
	if ($("#org").is(':checked')) {
		...
		if(($('input#url-fld').val()!='http://example.com or www.example.com') && !urlregex.test($('input#url-fld').val())){
			...
			if(($("input#url-fld").val()=="")) $("input#url-fld").val('http://example.com or www.example.com');
		}else{
			delErrorMsg($("input#url-fld"));
		}
	...
	if(inputError) {	
		...
		if((document.location.pathname.lastIndexOf("/")+1) == (document.location.pathname.length)){
			$("#ambsdr-form").attr("action","");
			}
		$("#ambsdr-form").submit();
		return true;
	}		
